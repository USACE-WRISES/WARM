  ...
  dashboardBody( #main body, describes whats inside each of the tabs above
    tabItems(
      ...
      tabItem(tabName="Bosque", #tab to look at the chosen model
              ...
              solidHeader = T,
              status = "primary",
              helpText("For the following, please enter the SI values associated with the respective inputs"),
      ...
      tabItem(tabName="SMURF", #tab to look at the chosen model
              ...
              solidHeader = T,
              status = "primary",
              helpText("hyd.att = longitudinal connectivity associated with flow attentuation"),
              helpText("stripwidth.ft = hydrologic and water quality benefits associated with a wide buffer strip"),
              helpText("flowpath.score = preferential flowpaths through the riparian buffer strip"),
              helpText("shading.ratio = channel shading"),
              helpText("cancov.score = canopy coverage of the stream"),
              helpText("canstr.score = canopy structure of the riparian forest"),
              helpText("deadfall.score = fallen logs in the riparian zone"),
              helpText("snag.score = standing deadwood suitable for roosting habitat"),
              helpText("batcan.score = canopy structure of the riparian forest relative to bat flight"),
              helpText("embed.score = stream embeddedness"),
              helpText("detritus.score = detrital leaf fall"),
              helpText("herb.score = herbaceous vegetation cover"),
              helpText("inv.veg.score = vegetation community composition within the riparian zone"),
              helpText("buffer.dev.score = buffer development"),
              helpText("edge.density.perft = hydrologic and water quality benefits associated with a wide buffer strip"),
              helpText("corridorwidth.ft = hydrologic and water quality benefits associated with a wide buffer strip"),
              helpText("corridormin.ft = local confinement of the riparian zone relative to movement corridors"),
